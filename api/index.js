...
// Session Middleware untuk Vercel (memory-based, nggak persist)
// Untuk production, gunakan Redis atau database session store
app.use(session({
  ...
  cookie: {
    httpOnly: true,
    secure: true, // HTTPS only di Vercel
    sameSite: 'lax',
...
app.post('/api/attendance/clock-in', (req, res) => {
  try {
    ...
    `).run(userId, today, lat, lng, distance, status);
...
app.post('/api/attendance/clock-out', (req, res) => {
  try {
    ...
    `).run(lat, lng, distance, userId, today);
